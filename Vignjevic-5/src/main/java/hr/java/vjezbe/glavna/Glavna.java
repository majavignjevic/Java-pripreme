...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        Sveuciliste<ObrazovnaUstanova> ustanove = null; //?
        ...
        for (m = 0; m < brojObrazovnihUstanova; m++) { //unosimo podatke za n obrazovnih ustanova, vrti sve:/
            ...
            for (i = 0; i < brojPredmeta; i++) {
                ...
                for(k=0;k<brStudenata;k++){
                    ...
                    System.out.print("\nOdaberite studenta: >>");
                    Integer brStud = unosBroja(unos, "Unos studenta na predmetu: ");
            ...
            //ispisujemo profesore i njihove kolegije na kojima predaju
            ...
            for (i = 0; i < brojIspitnihRokova; i++) {
                ...
                Integer brojStud = unosBroja(unos, "Unos broja studenata na roku");
            ...
            //filtritamo ispite po studentima, gledamo tko je ostvario odlican i onda ga filtriramo u listu filtriranih
            ispiti.stream()
                    .filter(ispit -> ispit.getOcjena().getNumber().equals(5))
                    .forEach(ispit -> System.out.println("Student " + ispit.getStud().getIme() + " " + ispit.getStud().getPrezime()
                    + "je ostvario ocjenu izvrstan na predmetu " + ispit.getPred().getNaziv()));

            //pitamo orisnika gdje hoce da mu unesemo podatke u koju ustanovu
            ...
            if (brojUstanove == 1) {
                ustanove.dodajObrazovnuUstanovu(new VeleucilisteJave(nazivUstanove, predmeti, profesori, studenti, ispiti));
            } else if (brojUstanove == 2) {           //instance of trazimo koji je Veleuciliste Jave ili Fakultet racunarstva
                ustanove.dodajObrazovnuUstanovu(new FakultetRacunarstva(nazivUstanove, predmeti, profesori, studenti, ispiti));
            } else {
        ...
        for(ObrazovnaUstanova ou : ustanove.dodajObrazovnuUstanovu()){           //provjeravamo postoji li vise najmladih studenata na fakultetu
        ...
        Comparator<ObrazovnaUstanova> poBrojuStudenata = Comparator.comparingInt(o -> o.getStudenti().size());

        List<ObrazovnaUstanova> sortiraneUstanove = ustanove.dodajObrazovnuUstanovu().stream()
                .sorted(poBrojuStudenata
