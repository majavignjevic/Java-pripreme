...
public class StudentiController implements Initializable {
    ...
    public void pretrazivanje(){
        ...
        double vrijednostSlidera= slider.getValue();
        System.out.println(vrijednostSlidera);
        ...
        if(vrijednostSlidera!=18 ||vrijednostSlidera!=40 ) {
            studenti = studenti.stream().filter(student -> student.getDatumRodjenja().getYear() - LocalDate.now().getYear() == vrijednostSlidera).collect(Collectors.toList());
        }
        jmbagStudentColumnTable.setCellValueFactory(new PropertyValueFactory<>("jmbag"));
        prezimeStudentaColumnTable.setCellValueFactory(new PropertyValueFactory<>("prezime"));
        imeStudentaColumnTable.setCellValueFactory(new PropertyValueFactory<>("ime"));
    ...
    @FXML
    public Double postaviBrojGodina(){
        Double vrijednostSlidera =slider.getValue();
        System.out.println(vrijednostSlidera);
        return vrijednostSlidera;
    }
}